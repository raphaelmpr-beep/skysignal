...
-- Add PostGIS geography column to salute_reports for UAS location
ALTER TABLE salute_reports
  ADD COLUMN l_uas_location GEOGRAPHY(POINT, 4326);

-- Computed geography from lat/lon via trigger
CREATE OR REPLACE FUNCTION salute_set_uas_location()
RETURNS TRIGGER AS $$
BEGIN
  IF NEW.l_uas_latitude IS NOT NULL AND NEW.l_uas_longitude IS NOT NULL THEN
    NEW.l_uas_location := ST_SetSRID(
      ST_MakePoint(NEW.l_uas_longitude, NEW.l_uas_latitude),
      4326
    )::GEOGRAPHY;
  END IF;
  RETURN NEW;
...
CREATE TRIGGER salute_reports_set_location
  BEFORE INSERT OR UPDATE ON salute_reports
  FOR EACH ROW EXECUTE FUNCTION salute_set_uas_location();

CREATE INDEX salute_reports_location_idx ON salute_reports USING GIST(l_uas_location);
